...
(function (window) {
  ...
  if (!window.Mishkah.UI) window.Mishkah.UI = {};

  const D = window.Mishkah.DSL;

  window.Mishkah.UI.WikiViewer = function (props) {
    const { db, wikiId, onNavigate } = props;
    const article = (window.codewikidb || []).find(a => a.id === wikiId);

    if (!article) {
      return D.Containers.Div({
        attrs: { class: 'p-8 text-center text-gray-500' }
      }, ['Article not found: ' + wikiId]);
    }

    const lang = db.env.lang || 'en';
    const title = article.title ? article.title[lang] : article.id;
    const content = article.content ? article.content[lang] : '';
    ...
    if (article.words && article.words.length > 0) {
      ...
      article.words.forEach(w => {
        ...
        wordMap[key.toLowerCase()] = w[key];
      });

      const pattern = new RegExp(`(?<!\\[|\\()\\b(${Object.keys(wordMap).join('|')})\\b(?!\\]|\\))`, 'gi');

      processedContent = content.replace(pattern, (match) => {
        const lower = match.toLowerCase();
        if (wordMap[lower]) {
          return `[${match}](#wiki:${wordMap[lower]})`;
        }
        return match;
      });
    }
    ...
    return D.Containers.Div({
      attrs: {
        ...
        id: 'wiki-viewer-container'
      }
    }, [
      D.Containers.Div({
        attrs: { class: 'p-4 border-b flex items-center gap-2 text-sm text-gray-500' }
      }, [
        ...(article.parents_ids || []).map(pid =>
          D.Text.Span({ attrs: { class: 'flex items-center gap-1' } }, [
            M.UI.Button({
              variant: 'link',
              size: 'sm',
              attrs: {
                class: 'p-0 h-auto font-normal',
                onclick: () => onNavigate && onNavigate(pid)
              }
            }, [pid]),
            D.Text.Span({}, ['/'])
          ])
        ),
        D.Text.Span({ attrs: { class: 'font-bold text-gray-900' } }, [title])
      ]),

      D.Containers.Div({
        attrs: {
          class: 'flex-1 overflow-auto p-8 prose max-w-none',
          onclick: (e) => {
        ...
        D.Text.H1({ attrs: { class: 'text-3xl font-bold mb-6' } }, [title]),

        D.Containers.Div({
          attrs: { class: 'wiki-content' }
        }, [
          window.Mishkah.UI.Markdown
            ? window.Mishkah.UI.Markdown({ content: processedContent })
            : (window.marked ? D.RawHtml({ html: window.marked.parse(processedContent) }) : processedContent)
        ])
      ]),

      (article.siblings && article.siblings.length > 0) ? D.Containers.Div({
        attrs: { class: 'p-4 border-t bg-gray-50' }
      }, [
        D.Text.H3({ attrs: { class: 'text-sm font-bold mb-2 uppercase text-gray-500' } }, [
          lang === 'ar' ? 'مواضيع ذات صلة' : 'Related Topics'
        ]),
        D.Containers.Div({ attrs: { class: 'flex flex-wrap gap-2' } },
          article.siblings.map(sib =>
            M.UI.Button({
              variant: 'outline',
              size: 'sm',
              attrs: {
                onclick: () => onNavigate && onNavigate(sib.id)
              }
            }, [sib.title[lang]])
          )
        )
...
(function (window) {
  ...
  window.Mishkah.UI.WikiSidebar = function (props) {
    const { db, onNavigate, onSearch } = props;
    const articles = window.codewikidb || [];
    const lang = db.env.lang || 'en';
    const searchQuery = (db.wikiSearch || '').toLowerCase();

    // Search Input
    const SearchInput = window.Mishkah.UI.Input({
      attrs: {
        type: 'text',
        placeholder: lang === 'ar' ? 'بحث...' : 'Search...',
        value: db.wikiSearch || '',
        class: 'w-full px-3 py-2 rounded border mb-4 text-sm',
    ...
    const renderTree = (parentId = null, depth = 0) => {
      const children = articles.filter(a => {
        if (parentId === null) return !a.parents_ids || a.parents_ids.length === 0;
        return a.parents_ids && a.parents_ids.includes(parentId);
      }).sort((a, b) => (a.sort || 0) - (b.sort || 0));

      if (children.length === 0) return null;

      return D.Containers.Div({
        attrs: { class: depth > 0 ? 'ml-4 border-l pl-2' : '' }
      }, children.map(article => {
        const isActive = db.activeWikiId === article.id;
        const title = article.title ? article.title[lang] : article.id;
        const hasChildren = articles.some(a => a.parents_ids && a.parents_ids.includes(article.id));
        ...
        return D.Containers.Div({}, [
          M.UI.Button({
            ...
            size: 'sm',
            attrs: {
              class: 'w-full text-left justify-start mb-1',
              onclick: () => onNavigate && onNavigate(article.id)
            }
          }, [
            hasChildren ? (lang === 'ar' ? '📂 ' : '📁 ') : (lang === 'ar' ? '📄 ' : '📄 '),
            title
          ]),
          renderTree(article.id, depth + 1)
        ]);
      }));
    };
    ...
    const renderSearchResults = () => {
      const matches = articles.filter(a => {
        const title = a.title ? a.title[lang] : a.id;
        return title.toLowerCase().includes(searchQuery) ||
          (a.keywords && a.keywords.some(k => k.toLowerCase().includes(searchQuery)));
      });

      if (matches.length === 0) {
        return D.Containers.Div({ attrs: { class: 'text-sm text-gray-500 text-center py-4' } },
          [lang === 'ar' ? 'لا توجد نتائج' : 'No results found']);
      }

      return D.Containers.Div({ attrs: { class: 'space-y-1' } }, matches.map(article => {
        const isActive = db.activeWikiId === article.id;
        const title = article.title ? article.title[lang] : article.id;
        return M.UI.Button({
          variant: isActive ? 'default' : 'ghost',
    ...
    return D.Containers.Div({
      attrs: { class: 'flex flex-col h-full' }
    }, [
      D.Containers.Div({ attrs: { class: 'p-4 border-b' } }, [SearchInput]),
      D.Containers.Div({ attrs: { class: 'flex-1 overflow-auto p-4' } }, [
