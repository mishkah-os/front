...
(function (root, factory) {
    ...
    var Logger = {
        log: function (msg, data) {
            if (typeof console !== 'undefined' && console.log) {
                try { console.log('[Mishkah.React]', msg, data || ''); } catch (err) { }
            }
        },
        error: function (msg, err) {
            if (typeof console !== 'undefined' && console.error) {
                try {
                    console.error('[ERROR]', msg, err && err.message ? err.message : err);
    ...
    // -------------------------------------------------------------------
    // Small helpers (ES5 compatibility)
    // -------------------------------------------------------------------
    function extendES5(target) {
        if (!target) target = {};
    ...
    function endsWithES5(str, suffix) {
        ...
        var instance = {
            ...
            render: function () {
                ...
                try {
                    ...
                    vnode = instance.ComponentFn(instance.props, children);
                    Logger.log('render:component', { name: ComponentFn && ComponentFn.name, props: instance.props });
                } catch (err) {
    ...
    // Context System (ES5-compatible store)
    var contextRegistry = {};
    ...
    function createContext(defaultValue) {
        ...
        var Context = {
            ...
            Provider: function (props) {
                contextRegistry[ctxId] = props.value;
    ...
    function useContext(Context) {
        if (Context && Context.id && contextRegistry.hasOwnProperty(Context.id)) {
            return contextRegistry[Context.id];
        }
    ...
    function html(strings) {
            ...
            if (spreadMatch && i < values.length) {
                ...
                if (i < values.length) {
                    ...
                    if (lastChar === '<') {
                        ...
                        // prop=${val}
                        var id = addPlaceholder(val);
                        if (endsWithES5(trimmed, '="') || endsWithES5(trimmed, "='")) {
                            out += "m-ph-" + id;
                        } else {
                            out += '"m-ph-' + id + '"';
                        }
        ...
        function buildVNode(tag, node) {
            ...
            for (var i = 0; i < node.attributes.length; i++) {
                ...
                if (name === 'data-m-spread') {
                    ...
                    props = extendES5(props, spreadProps);
                    continue;
                }

                if (typeof val === 'string' && startsWithES5(val, 'm-ph-')) {
        ...
        if (nodes.length > 0) {
            var nodeList = [];
            for (var k = 0; k < nodes.length; k++) nodeList.push(domToVNode(nodes[k]));
            return nodeList;
        }
        return null;
    }
    ...
    function createElement(type, props) {
        ...
        for (var argIndex = 2; argIndex < arguments.length; argIndex++) {
            children.push(arguments[argIndex]);
        }
        flatten(children);

        // If type is a function (component)
        if (typeof type === 'function') {
            var componentProps = extendES5({}, props, { children: flatChildren });
            try {
                return createComponent(type, componentProps).render();
            } catch (err) {
                Logger.error('createElement component failed', err);
                throw err;
            }
        }
