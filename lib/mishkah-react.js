...
(function (root, factory) {
    ...
    function html(strings) {
        ...
        function pushComponentPlaceholder(val) {
            var id = addPlaceholder(val);
            componentStack.push({ value: val, id: id });
            return id;
        }

        function popComponentPlaceholder(val) {
            for (var idx = componentStack.length - 1; idx >= 0; idx--) {
                if (componentStack[idx].value === val) {
                    var match = componentStack[idx].id;
                    componentStack.splice(idx, 1);
                    return match;
                }
            }
            return addPlaceholder(val);
        }
            ...
            if (spreadMatch && i < values.length) {
                ...
                if (i < values.length) {
                    ...
                    if (lastChar === '<') {
                        ...
                        var id = pushComponentPlaceholder(val);
                        out += "<m-component-" + id;

                        // Self-closing check: if the immediate next string starts with '/>' (ignoring whitespace),
                        // we won't see a separate closing tag interpolation. Pop the placeholder now to avoid
                        // leaking stack entries when using repeated components of the same type.
                        var nextStr = strings[i + 1] || "";
                        if (/^\s*\/>/.test(nextStr)) {
                            componentStack.pop();
                        }
                        ...
                        if (outTrimmed.endsWith('</')) {
                            ...
                            var id = popComponentPlaceholder(val);
                            out += "</m-component-" + id + ">";
                        } else {
    ...
    function createElement(type, props) {
        ...
        /* FIXED */ var children = []; for (var _i = 2; _i < arguments.length; _i++) { children.push(arguments[_i]); } flatten(children);
